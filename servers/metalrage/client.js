...
const { pack, unpack } = require('./message.js');
...
class NetworkClient 
{
    ...
    getLocalTime()
    {
        // Is the Date constructor expensive in JavaScript, is there
        // some kind of stopwatch I could use instead?
    ...
    onData(data)
    {
        // Should probably disconnect if we're unable to
        // unpack or verify the message?
        if (!unpack(data, this.salt_)) return;

        const len = data.readUint16BE(0x6);
        const type = data.readUint32BE(0xC);
        const body = data.subarray(0x10, len);
        
        // Don't want to keep printing MSG_KEEP_ALIVE to console.
        if (type != 0x20083)
        {
            console.log(`[NetworkClient] RECV: 0x${type.toString('16')} w/ len = 0x${len.toString('16')}`);
            console.log(data);
        }

        // Handle internal dispatch events
        if (type & 0x80)
        {
